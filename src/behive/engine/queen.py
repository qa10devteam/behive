"""Queen Planner — the intelligence brain that plans the scout swarm.

Extracted from orchestrator.py for maintainability.
The Queen analyzes the topic and generates 200+ scout tasks for maximum coverage.
"""
import os
import sys
import json
...
try:
    from behive.engine.llm import call_llm, complete
except ImportError:
    pass
...
Return ONLY the JSON array."""
        ...
        text = complete(prompt, stage="scout", system=system_prompt, max_tokens=4096)
        if text.startswith('```'):
...
Be factual for the profile. For competitor list — use domain knowledge about the industry."""
        ...
        try:
            profile = complete(prompt, stage="scout", max_tokens=1500)
            return f"\n\n═══ SUBJECT INTELLIGENCE PROFILE ═══\n{profile}\n═══════════════════════════════════\n"
        except Exception as e:
            log.debug(f"Exception in orchestrator.py: {e}")
            # LLM failed — return raw truncated text
...
Return ONLY the JSON array."""
        ...
        try:
            text = complete(prompt, stage="scout", max_tokens=2000)
            if text.startswith('```'):
                text = text.split('\n', 1)[1].rsplit('```', 1)[0].strip()
